...
pub mod coding {
    ...
    pub async fn run_coding_agent(
        ...
        let cfg = clone_config_with_optional_role(ctx.config, model_role).context("config clone")?;
        ...
        codex
            .submit(Op::UserInput {
                items: vec![InputItem::Text { text: task.to_string() }],
            })
            .await
...
pub mod file_search {
    use super::*;
    use codex_file_search::{run, FileSearchResults};
    use std::num::NonZero;
    use std::path::Path;
    ...
    pub async fn run_file_search_agent(
        ...
        let FileSearchResults { matches, total_match_count } = run(
            query,
            limit,
            Path::new(search_dir),
            vec![],
...
pub mod web_search {
    ...
    pub async fn run_web_search_agent(
        ...
        let provider = std::env::var("CODEX_WEBSEARCH_PROVIDER").unwrap_or_else(|_| "duckduckgo".to_string());
        let k = top_k.unwrap_or(5);
        let urls = match provider.as_str() {
            "google_cse" => {
                let key = std::env::var("CODEX_GOOGLE_CSE_KEY").map_err(|_| anyhow!("CODEX_GOOGLE_CSE_KEY not set"))?;
                let cx = std::env::var("CODEX_GOOGLE_CSE_CX").map_err(|_| anyhow!("CODEX_GOOGLE_CSE_CX not set"))?;
...
pub mod rag {
    use super::*;
    use crate::web::{crawl_and_extract, ddg_search, WebDoc};
    use std::fs;
    use std::path::Path;
    ...
    pub async fn run_rag_agent(
                ...
                for entry in walkdir::WalkDir::new(root).max_depth(5).into_iter().filter_map(|e| e.ok()) {
                    ...
                    if p.is_file() && added < 200 {
                        if let Ok(content) = fs::read_to_string(p) {
                            if content.chars().any(|c| c == '\u{0}') { continue; }
        ...
        if let Some(base) = base {
            let q_vec = embed_texts(&client, &base, &[question.to_string()]).await?.pop().unwrap_or_default();
            for (prov, text) in contexts.iter() {
                for chunk in chunk_text(text, 2000) {
                    let v = embed_texts(&client, &base, &[chunk.clone()]).await?.pop().unwrap_or_default();
        ...
        codex
            .submit(codex_protocol::protocol::Op::UserInput { items: vec![codex_protocol::protocol::InputItem::Text { text: question.to_string() }] })
            .await?;
        ...
        Ok(AgentResult { success: true, summary: answer.unwrap_or_else(|| "RAG answer available".to_string()), details: Some(details) })
    }

    async fn embed_texts(client: &reqwest::Client, base_url: &str, texts: &[String]) -> Result<Vec<Vec<f32>>> {
        #[derive(Serialize)]
        struct Req { texts: Vec<String> }
        #[derive(Deserialize)]
        struct Resp { embeddings: Vec<Vec<f32>> }
        let url = format!("{}/v1/embeddings", base_url.trim_end_matches('/'));
        let resp: Resp = client.post(url).json(&Req { texts: texts.to_vec() }).send().await?.json().await?;
        Ok(resp.embeddings)
    }

    fn chunk_text(s: &str, max_chars: usize) -> Vec<String> {
        if s.len() <= max_chars { return vec![s.to_string()] };
        let mut out = Vec::new();
